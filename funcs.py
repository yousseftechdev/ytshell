...
def get_prompt():
    ...
    if os.getuid() == 0:
        if timeInPrompt.lower() in ["t", "true"]:
            if exitCode != -200:
                if exitCode == 0:
                    prompt = f"╭─{termcolor.colored('', 'red') + termcolor.colored(' ' + os.path.expanduser('~').split('/')[1] + f'@{os.uname().nodename} 💀 [' + fdir, 'white', 'on_red') + termcolor.colored(currentFolder.replace('root', ''), 'white', 'on_red', attrs=['bold']) + termcolor.colored(' ] ', 'white', 'on_red') + termcolor.colored('', 'red', f'on_{git_cross_color}')}{git_prompt}···{termcolor.colored('', statBg) + termcolor.colored(' ✔ ', statTxt, f'on_{statBg}') + termcolor.colored('', timeBg, f'on_{statBg}') + termcolor.colored(' ' + ftime + ' ', timeTxt, f'on_{timeBg}') + termcolor.colored('', timeBg)}\n│\n╰─ {promptChar} " if git_prompt else f"╭─{termcolor.colored('', 'red') + termcolor.colored(' ' + os.path.expanduser('~').split('/')[1] + f'@{os.uname().nodename} 💀 [' + fdir, 'white', 'on_red') + termcolor.colored(currentFolder.replace('root', ''), 'white', 'on_red', attrs=['bold']) + termcolor.colored(' ] ', 'white', 'on_red') + termcolor.colored('', 'red')}···{termcolor.colored('', statBg) + termcolor.colored(' ✔ ', statTxt, f'on_{statBg}') + termcolor.colored('', timeBg, f'on_{statBg}') + termcolor.colored(' ' + ftime + ' ', timeTxt, f'on_{timeBg}') + termcolor.colored('', timeBg)}\n│\n╰─ {promptChar} " # type: ignore
                else:
                    prompt = f"╭─{termcolor.colored('', 'red') + termcolor.colored(' ' + os.path.expanduser('~').split('/')[1] + f'@{os.uname().nodename} 💀 [' + fdir, 'white', 'on_red') + termcolor.colored(currentFolder.replace('root', ''), 'white', 'on_red', attrs=['bold']) + termcolor.colored(' ] ', 'white', 'on_red') + termcolor.colored('', 'red', f'on_{git_cross_color}')}{git_prompt}···{termcolor.colored('', statErrBg) + termcolor.colored(f' {exitCode} ✘ ', statErrTxt, f'on_{statErrBg}') + termcolor.colored('', timeBg, f'on_{statErrBg}') + termcolor.colored(' ' + ftime + ' ', timeTxt, f'on_{timeBg}') + termcolor.colored('', timeBg)}\n│\n╰─ {promptChar} " if git_prompt else f"╭─{termcolor.colored('', 'red') + termcolor.colored(' ' + os.path.expanduser('~').split('/')[1] + f'@{os.uname().nodename} 💀 [' + fdir, 'white', 'on_red') + termcolor.colored(currentFolder.replace('root', ''), 'white', 'on_red', attrs=['bold']) + termcolor.colored(' ] ', 'white', 'on_red') + termcolor.colored('', 'red')}···{termcolor.colored('', statErrBg) + termcolor.colored(f' {exitCode} ✘ ', statErrTxt, f'on_{statErrBg}') + termcolor.colored('', timeBg, f'on_{statErrBg}') + termcolor.colored(' ' + ftime + ' ', timeTxt, f'on_{timeBg}') + termcolor.colored('', timeBg)}\n│\n╰─ {promptChar} " # type: ignore
            else:
                prompt = f"╭─{termcolor.colored('', 'red') + termcolor.colored(' ' + os.path.expanduser('~').split('/')[1] + f'@{os.uname().nodename} 💀 [' + fdir, 'white', 'on_red') + termcolor.colored(currentFolder.replace('root', ''), 'white', 'on_red', attrs=['bold']) + termcolor.colored(' ] ', 'white', 'on_red') + termcolor.colored('', 'red', f'on_{git_cross_color}')}{git_prompt}···{termcolor.colored('', timeBg) + termcolor.colored(' ' + ftime + ' ', timeTxt, f'on_{timeBg}') + termcolor.colored('', timeBg)}\n│\n╰─ {promptChar} " if git_prompt else f"╭─{termcolor.colored('', 'red') + termcolor.colored(' ' + os.path.expanduser('~').split('/')[1] + f'@{os.uname().nodename} 💀 [' + fdir, 'white', 'on_red') + termcolor.colored(currentFolder.replace('root', ''), 'white', 'on_red', attrs=['bold']) + termcolor.colored(' ] ', 'white', 'on_red') + termcolor.colored('', 'red')}···{termcolor.colored('', timeBg) + termcolor.colored(' ' + ftime + ' ', timeTxt, f'on_{timeBg}') + termcolor.colored('', timeBg)}\n│\n╰─ {promptChar} " # type: ignore
        else:
            if exitCode != -200:
                if exitCode == 0:
                ...
                prompt = f"╭─{termcolor.colored('', 'red') + termcolor.colored(' ' + os.path.expanduser('~').split('/home/')[1] + f'@{os.uname().nodename} - [' + fdir, 'white', 'on_red') + termcolor.colored(currentFolder, promptTxt, f'on_{promptBg}', attrs=['bold']) + termcolor.colored(' ] ', promptTxt, f'on_{promptBg}') + termcolor.colored('', promptBg, f'on_{git_cross_color}')}{git_prompt}\n│\n╰─ {promptChar} " if git_prompt else f"╭─{termcolor.colored('', 'red') + termcolor.colored(' ' + os.path.expanduser('~').split('/home/')[1] + f'@{os.uname().nodename} - [' + fdir, 'white', 'on_red') + termcolor.colored(currentFolder, promptTxt, f'on_{promptBg}', attrs=['bold']) + termcolor.colored(' ] ', promptTxt, f'on_{promptBg}') + termcolor.colored('', promptBg)}\n│\n╰─ {promptChar} " # type: ignore

    else:
        if timeInPrompt.lower() in ["t", "true"]:
            if exitCode != -200:
                if exitCode == 0:
                    prompt = f"╭─{termcolor.colored('', promptBg) + termcolor.colored(' ' + os.path.expanduser('~').split('/')[2] + f'@{os.uname().nodename} [' + fdir, promptTxt, f'on_{promptBg}') + termcolor.colored(currentFolder, promptTxt, f'on_{promptBg}', attrs=['bold']) + termcolor.colored(' ] ', promptTxt, f'on_{promptBg}') + termcolor.colored('', promptBg, f'on_{git_cross_color}')}{git_prompt}···{termcolor.colored('', statBg) + termcolor.colored(' ✔ ', statTxt, f'on_{statBg}') + termcolor.colored('', timeBg, f'on_{statBg}') + termcolor.colored(' ' + ftime + ' ', timeTxt, f'on_{timeBg}') + termcolor.colored('', timeBg)}\n│\n╰─ {promptChar} " if git_prompt else f"╭─{termcolor.colored('', promptBg) + termcolor.colored(' ' + os.path.expanduser('~').split('/')[2] + f'@{os.uname().nodename} [' + fdir, promptTxt, f'on_{promptBg}') + termcolor.colored(currentFolder, promptTxt, f'on_{promptBg}', attrs=['bold']) + termcolor.colored(' ] ', promptTxt, f'on_{promptBg}') + termcolor.colored('', promptBg)}···{termcolor.colored('', statBg) + termcolor.colored(' ✔ ', statTxt, f'on_{statBg}') + termcolor.colored('', timeBg, f'on_{statBg}') + termcolor.colored(' ' + ftime + ' ', timeTxt, f'on_{timeBg}') + termcolor.colored('', timeBg)}\n│\n╰─ {promptChar} " # type: ignore
                else:
            ...
            if exitCode != -200:
                ...
                prompt = f"╭─{termcolor.colored('', promptBg) + termcolor.colored(' ' + os.path.expanduser('~').split('/')[2] + f'@{os.uname().nodename} [' + fdir, promptTxt, f'on_{promptBg}') + termcolor.colored(currentFolder, promptTxt, f'on_{promptBg}', attrs=['bold']) + termcolor.colored(' ] ', promptTxt, f'on_{promptBg}') + termcolor.colored('', promptBg, f'on_{git_cross_color}')}{git_prompt}\n│\n╰─ {promptChar} " if git_prompt else f"╭─{termcolor.colored('', promptBg) + termcolor.colored(' ' + os.path.expanduser('~').split('/')[2] + f'@{os.uname().nodename} [' + fdir, promptTxt, f'on_{promptBg}') + termcolor.colored(currentFolder, promptTxt, f'on_{promptBg}', attrs=['bold']) + termcolor.colored(' ] ', promptTxt, f'on_{promptBg}') + termcolor.colored('', promptBg)}\n│\n╰─ {promptChar} " if git_prompt else f"╭─{termcolor.colored('', promptBg) + termcolor.colored(' ' + os.path.expanduser('~').split('/')[1] + f'@{os.uname().nodename} [' + fdir, promptTxt, f'on_{promptBg}') + termcolor.colored(currentFolder, promptTxt, f'on_{promptBg}', attrs=['bold']) + termcolor.colored(' ] ', promptTxt, f'on_{promptBg}') + termcolor.colored('', promptBg)}\n│\n╰─ {promptChar} " if git_prompt else f"╭─{termcolor.colored('', promptBg) + termcolor.colored(' ' + os.path.expanduser('~').split('/')[1] + f'@{os.uname().nodename} [' + fdir, promptTxt, f'on_{promptBg}') + termcolor.colored(currentFolder, promptTxt, f'on_{promptBg}', attrs=['bold']) + termcolor.colored(' ] ', promptTxt, f'on_{promptBg}') + termcolor.colored('', promptBg)}\n│\n╰─ {promptChar} "# type: ignore
    exitCodeFile.close()
    return prompt
def get_ftime():
    configFile = open(f"{os.path.expanduser('~')}/.config/ytshell/config.txt", "r")
